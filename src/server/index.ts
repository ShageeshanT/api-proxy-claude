...
import { handleChatCompletions, handleModels, handleHealth, handleRoot, handleChat } from "./routes.js";
...
function createApp(): Express {
  ...
  // Bearer-token auth for the /v1 API when PROXY_API_KEY is set.
  // /, /health, and OPTIONS are always open so browsers and Railway
...
export async function startServer(config: ServerConfig): Promise<Server> {
  // Default to 0.0.0.0 when HOST env var is set (container deploys), else loopback.
  const defaultHost = process.env.HOST || (process.env.PORT ? "0.0.0.0" : "127.0.0.1");
  const { port, host = defaultHost } = config;
