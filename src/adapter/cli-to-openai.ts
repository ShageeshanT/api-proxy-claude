...
export function cliResultToOpenai(
  ...
  // Claude Code reports per-model usage in modelUsage and may delegate subtasks
  // to Haiku for planning. Prefer the "primary" tier (opus > sonnet > haiku) so
  // the reported model reflects what handled the user-facing reply.
  const usedModels = result.modelUsage ? Object.keys(result.modelUsage) : [];
  const primary =
    usedModels.find((m) => m.includes("opus")) ??
    usedModels.find((m) => m.includes("sonnet")) ??
    usedModels[0];
  const modelName = primary ?? "claude-opus-4-6";
...
function normalizeModelName(model: string | undefined): string {
  ...
  // Preserve version specificity: "claude-opus-4-6-20251010" -> "claude-opus-4-6"
